...
proj_results <- foreach(
  ...
  .export = c("run_la", "generate_ss", "measures")
) %dopar% {
  #run_projpred_one(i, ndev, n.para, beta0, beta, nval)
  run_la(i, ndev, nval, n.para, beta0, beta)
}
...
# Save output
write.csv(proj_results, "proj_la_results.csv", row.names = FALSE)
saveRDS(proj_results, "proj_la_results.rds")
